...
class PacbotCommsModule(rm.ProtoModule):
    def __init__(self, server_addr, server_port, game_engine_addr, game_engine_port, local_addr, local_port):
        ...
        # this connects to the game engine
        # self.game_engine_module = PacbotGameEngineClient(game_engine_addr, game_engine_port)
    ...
    def tick(self):
        ...
        # state = self.game_engine_module.get_state()
        # if state != None:
        #     # Broadcast command to local modules
        #     self.write(state.SerializeToString(), MsgType.PACMAN_COMMAND)

def main():
    module = PacbotCommsModule(SERVER_ADDRESS, SERVER_PORT, GAME_ENGINE_ADDRESS, GAME_ENGINE_PORT, LOCAL_ADDRESS, LOCAL_PORT)
    module.run()
